...
| 키 | 설명 |
|----|------|
| `intent_match` | 멀티 인텐트 Jaccard 유사도 |
...
현재는 `ground_truth.intents`를 기준으로 Jaccard 유사도를 계산합니다.  
이메일에 여러 인텐트가 포함된 경우(예: `policy_qna` + `reservation_create`)를 평가에 반영합니다.

```python
# 현재
predicted = set(outputs.get("intents") or [])
expected  = set(ref.get("intents") or [])
