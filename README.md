...
`spam` / `high urgency` 메일은 자동 처리하지 않고 즉시 매니저 승인 단계로 전달합니다.  
오류가 발생해도 즉시 실패 처리하지 않고 state에 기록한 뒤 매니저 승인 단계로 전달합니다.  

관리자 UI에서는 에이전트가 생성한 분류 결과, 답변 초안, 참고 SQL을 확인하고 수정할 수 있습니다.  
매니저가 승인하거나 수정 내용을 제출하면 FastAPI가 중단된 LangGraph 실행을 `resume`하여 이후 메일 발송 단계로 진행합니다.

모든 케이스에 대하여 매니저 승인노드로 보내는 이유는 LLM을 최종 결정자로 두지 않고, 오분류·정보 누락·부적절한 답변 가능성을 매니저가 언제든 수정할 수 있도록 하기 위한 HITL 설계입니다.
